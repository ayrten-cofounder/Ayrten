...
public class Level 
{
	...
	private void gen_grn_dots()
	{
		...
		for(int i = 0; i < num; i++)
		{
			GreenDot dot = new GreenDot(grn_texture);
			dot.setPosition(random.nextInt(width), random.nextInt(height));
			greenDots.add(dot);
		}
	}
	...
	private void gen_red_dots()
	{
		...
		for(int i = 0; i < num; i++)
		{
			RedDot dot = new RedDot(red_texture);
			dot.setPosition(random.nextInt(width), random.nextInt(height));
			redDots.add(dot);
		}
	}
	...
	private void gen_blue_dots()
	{
		...
		for(int i = 0; i < num; i++)
		{
			BlueDot dot = new BlueDot(blu_texture);
			dot.setPosition(random.nextInt(width), random.nextInt(height));
			blueDots.add(dot);
		}
	}
