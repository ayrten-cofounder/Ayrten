...
public class MessageScreen extends ScrotsScreen {
	...
	public MessageScreen(Table top_table, int pages)
	{
		...
		previous.setBounds(previous.getX(), previous.getY(), previous.getWidth(), previous.getHeight());
		previous.addListener(new ClickListener(){			
			@Override
			public void clicked(InputEvent event, float x, float y) {
				if(message_scroll.getScrollX() != 0)
					message_scroll.scrollTo(message_scroll.getScrollX() - Assets.width, 0, message_scroll.getWidth(), message_scroll.getHeight());
			}
		});
		
		Label next = new Label("Next", Assets.style_font_64_orange);
		next.setBounds(next.getX(), next.getY(), next.getWidth(), next.getHeight());
		...
		table.add(message_scroll).height(Assets.height - next.getStyle().font.getLineHeight());
		table.row();
	...
	// Needs to be overridden, should be called when transitioning from MessageScreen
	// to another screen.
